...
const SIGNAL_FIELD_MAP={
    ...
    // WARNING: tire pressure slot mapping is UNCONFIRMED and conflicting
    // between sources. markoceri/leapmotor-api's docs assign these same four
    // signal IDs differently (2667=leftFront, 2646=leftRear, 2660=rightRear),
    // and leapmotor-ha's changelog independently reports fixing a bug where
    // "rear right pressure displayed as front left" - i.e. exactly this kind
    // of mixup. Neither their corrected table nor ours has been physically
    // verified against a real vehicle (e.g. by inflating one tire to a
    // distinct pressure and checking which datapoint changes), so do NOT
    // change this without such verification - guessing risks swapping to a
    // different wrong mapping instead of fixing it.
