...
const STATUS_ENDPOINT_OVERRIDES={
    B10:'c10',
    // Confirmed via leapmotor-ha changelog (0.6.29/0.6.30, 2026-07-15): a
    // European B05 owner's live status pull through the shared C10 endpoint
    // returned 95 status signals, avoiding the HTTP 404 previously seen when
    // trying a model-specific "b05" path (see our issue #38).
    B05:'c10',
    // "B11" is not a separate model - it's Leapmotor's internal platform/
    // chassis code for the C10 itself (confirmed via ADAC vehicle database:
    // "Baureihe C10 (B11)"). Some sources/cloud responses surface this
    // internal code as carType instead of "C10" - map it the same way.
    B11:'c10',
    // C16 is suspected to share the same C10-family endpoint but this is
    // UNCONFIRMED - add/adjust here once verified by an owner.
};
...
const SIGNAL_FIELD_MAP={
    ...
    // GPS location: signals 2/3 are signed (retain West/South hemisphere
    // sign) and preferred; 3724/3725 and legacy 2190/2191 are absolute-value
    // variants used as fallbacks below. Community-confirmed via leapmotor-ha
    // - using only the absolute-value signals would show vehicles west of
    // Greenwich (UK, Ireland, Portugal, parts of ES/FR) mirrored into the
    // wrong hemisphere.
    '3':'latitudeSigned',
    '2':'longitudeSigned',
    '3725':'latitude',
    ...
    // Tire pressure slot mapping: CONFIRMED correct via leapmotor-ha's
    // changelog, which describes fixing this exact mapping after live C10
    // verification (previously mismapped tires showed rear-right pressure as
    // front-left) - their corrected, verified mapping matches ours exactly.
    // The earlier markoceri/leapmotor-api docs assigning these same four
    // signal IDs differently were apparently the unverified/incorrect source.
...
function mergeSignalToNamed(data){
    ...
    // GPS priority: signed (2/3) > absolute-value primary (3724/3725) >
    // legacy fallback (2190/2191). See SIGNAL_FIELD_MAP comment above.
    if(named.latitudeFallback!==undefined&&named.latitude===undefined)named.latitude=named.latitudeFallback;
    if(named.longitudeFallback!==undefined&&named.longitude===undefined)named.longitude=named.longitudeFallback;
    if(named.latitudeSigned!==undefined)named.latitude=named.latitudeSigned;
    if(named.longitudeSigned!==undefined)named.longitude=named.longitudeSigned;
    delete named.latitudeSigned;delete named.longitudeSigned;
    delete named.latitudeFallback;delete named.longitudeFallback;
...
class LeapmotorClient{
    ...
    _parseResponse(data,label){if(data.code!==0){const msg=String(data.message||JSON.stringify(data));throw new Error(`${label} failed: ${msg} (code=${data.code})`)}return data}
    _loadAccountCert(ld){
    ...
    async login(){
        // Fresh device identity on every login attempt (not just the very
        // first one). Reusing the same deviceId across retries after a
        // failed login can apparently get that identity rejected/poisoned
        // server-side - this mirrors what a full adapter restart does
        // (mints a brand-new random deviceId), without requiring one.
        this.deviceId=crypto.randomUUID().replace(/-/g,'');
        ...
        // Log the full raw response (minus sensitive fields) at debug level
        // whenever login isn't immediately successful - useful for diagnosing
        // upstream API changes (e.g. policy/version bumps) without exposing
        // the token/certs themselves.
        if(resp.data?.code!==0){
            const safe={...resp.data};
            if(safe.data)safe.data={...safe.data,base64Cert:safe.data.base64Cert?'[REDACTED]':undefined,token:safe.data.token?'[REDACTED]':undefined};
            this._adapter?.log?.debug(`Login non-success raw response: ${JSON.stringify(safe)}`);
        }
    ...
    async getVehicleStatus(vehicle){
        ...
        if(!this._rawStatusDumped.has(vehicle.vin)){
            ...
            // From the second poll onward, log only what CHANGED vs. the
            // previous poll - lets you toggle something in the vehicle/app
            // and see immediately, without manual copy/paste, which field(s)
            // reflect it. Flattened one level deep so nested signal/config
            // objects (C10/B10/B05) get per-key diffs instead of one opaque
            // "the whole signal object changed". Only emitted at debug level.
            const prev=this._lastRawStatus.get(vehicle.vin);
            if(prev){
                const flatten=(obj)=>{
                    const out={};
                ...
                const diff={};
                const keys=new Set([...Object.keys(prevFlat),...Object.keys(dataFlat)]);
                for(const k of keys){
                    if(JSON.stringify(prevFlat[k])!==JSON.stringify(dataFlat[k])){
                        diff[k]={from:prevFlat[k],to:dataFlat[k]};
                    }
                }
                if(Object.keys(diff).length>0){
                    this._adapter?.log?.debug(`${vehicle.vin}: raw status diff since last poll: ${JSON.stringify(diff)}`);
                }
            }
        }
        this._lastRawStatus.set(vehicle.vin,data);
    ...
    async getEnergyBreakdown(vehicle,beginSeconds,endSeconds){
        ...
        // One-time diagnostic: this endpoint's name ("getLastweekEC") suggests
        // it may be designed for week-level aggregates or a list of the
        // cloud's own driving records, not an arbitrary short window lookup.
        // Logged BEFORE the parse/throw below, since a "no data found" style
        // response (code=100) throws inside _parseResponse - a log placed
        // after that call never runs for exactly the failing case we most
        // want to inspect.
        if(!this._energyBreakdownRawLogged){
            this._energyBreakdownRawLogged=true;
            this._adapter?.log?.debug(`${vehicle.vin}: raw energy breakdown response (begin=${beginSeconds}, end=${endSeconds}): ${JSON.stringify(resp.data)}`);
        }
    ...
    // Official, device-metered charging session history with GPS per
    // session (start/end timestamp, AC/DC, kWh, lat/lon) - community
    // documented via markoceri/leapmotor-api's docs/api.md, 2026-09. More
    // accurate than our own poll-based SoC-delta estimate for home/public
    // classification, since it's the cloud's own metering, not derived from
    // periodic status snapshots. This is the one endpoint requiring a JSON
    // body instead of form-urlencoded - Content-Type must be set explicitly
    // per call since the rest of the client defaults to urlencoded.
    async getChargingDailyDetail(vehicle,startDate,endDate,pageNum=1,pageSize=50){
        const offsetMin=-new Date().getTimezoneOffset();
        const sign=offsetMin>=0?'+':'-';
        const abs=Math.abs(offsetMin);
        const timeZone=`GMT${sign}${String(Math.floor(abs/60)).padStart(2,'0')}:${String(abs%60).padStart(2,'0')}`;
        const bodyObj={vin:vehicle.vin,timeZone,startTime:startDate,endTime:endDate,pageNum,pageSize};
        const h={...this._h(buildSignedHeaders({signKey:this._signKey,deviceId:this.deviceId,language:this.config.language,bodyParams:{vin:vehicle.vin,timeZone,startTime:startDate,endTime:endDate,pageNum:String(pageNum),pageSize:String(pageSize)}})),...this._authHeaders(),'Content-Type':'application/json'};
        const resp=await this.http.post('/carownerservice/charge/daily/detail/page',bodyObj,{headers:h});
        return this._parseResponse(resp.data,'charging daily detail');
    }
