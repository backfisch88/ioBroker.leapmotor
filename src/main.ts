/**
 * ioBroker.leapmotor – Hauptadapter v0.2.0
 * Leapmotor T03 Integration
 */
...
class LeapmotorAdapter extends utils.Adapter {
    private client: LeapmotorClient | null = null;
    ...
    async onReady(): Promise<void> {
        ...
        const cfg = this.config as {
            email: string; password: string; operationPassword: string;
            pollingInterval: number; language: string;
        };
        if (!cfg.email || !cfg.password) { this.log.error('E-Mail und Passwort müssen konfiguriert sein!'); return; }
        let appCertPem: string, appKeyPem: string;
        try {
            appCertPem = fs.readFileSync(path.join(CERT_DIR, 'app.crt'), 'utf8');
            appKeyPem  = fs.readFileSync(path.join(CERT_DIR, 'app.key'), 'utf8');
        } catch (e) { this.log.error(`Zertifikate nicht gefunden: ${e}`); return; }
        this.client = new LeapmotorClient({
            username: cfg.email, password: cfg.password, appCertPem, appKeyPem,
            operationPassword: cfg.operationPassword || undefined,
            language: cfg.language || 'en-GB',
        });
        try {
            this.log.info('Verbinde mit Leapmotor Cloud...');
    ...
    private async pollAll(): Promise<void> {
        ...
        try {
            ...
            if (msg.includes('ungültig') || msg.includes('Token') || msg.includes('401')) {
                ...
                try {
                    await this.client.login();
                    this.log.debug('Re-Login erfolgreich.');
                    for (const v of this.vehicles) await this.updateVehicleStatus(v);
    ...
    private async updateVehicleStatus(vehicle: Vehicle): Promise<void> {
    ...
    private async updateConsumption(vehicle: Vehicle): Promise<void> {
        if (!this.client) return;
        const vin = vehicle.vin;
        try {
            const m = await this.client.getMileageEnergyDetail(vehicle);
            const d = (m.data ?? {}) as Record<string, unknown>;
            await this.ensureAndSet(`${vin}.consumption.mileage_total_km`, d.totalmileage as number, 'number', 'km', 'Gesamtkilometer');
            await this.ensureAndSet(`${vin}.consumption.delivery_days`, d.deliveryDays as number, 'number', 'Tage', 'Liefertage');
        } catch (e) { this.log.debug(`Mileage: ${e}`); }
        try {
            const w = await this.client.getConsumptionWeeklyRank(vehicle);
            const d = (w.data ?? {}) as Record<string, unknown>;
            const rank = ((d.rankResult ?? d.rank ?? {}) as Record<string, unknown>);
            await this.ensureAndSet(`${vin}.consumption.kwh_100km`, (rank.hundredKmEC ?? rank.hundredKmEc) as number, 'number', 'kWh/100km', 'Ø Verbrauch');
            await this.ensureAndSet(`${vin}.consumption.rank`, rank.rank as string, 'string', '', 'Ranking');
            const weekly = ((d.weeklyEC ?? d.weekly ?? []) as Record<string, unknown>[]);
            for (let i = 0; i < weekly.length; i++) {
                const wb = `${vin}.consumption.week_${i + 1}`;
                await this.ensureAndSet(`${wb}.week_start`, (weekly[i].weekStart ?? weekly[i].week_start) as string, 'string', '', 'Start');
                await this.ensureAndSet(`${wb}.week_end`,   (weekly[i].weekEnd   ?? weekly[i].week_end)   as string, 'string', '', 'Ende');
                await this.ensureAndSet(`${wb}.kwh_100km`,  (weekly[i].hundredKmEC ?? weekly[i].hundredKmEc) as number, 'number', 'kWh/100km', 'Verbrauch');
            }
        } catch (e) { this.log.debug(`Weekly: ${e}`); }
    }

    private async ensureAndSet(id: string, val: unknown, type: string, unit: string, name: string): Promise<void> {
        if (val === null || val === undefined) return;
        await this.setObjectNotExistsAsync(id, { type: 'state', common: { name, type: type as ioBroker.CommonType, unit, role: 'value', read: true, write: false }, native: {} });
        await this.setStateAsync(id, { val: val as ioBroker.StateValue, ack: true });
    }

    // ── Pictures ─────────────────────────────────────────────

    private async updatePictures(vehicle: Vehicle): Promise<void> {
        if (!this.client) return;
        const vin = vehicle.vin;
        let cache: Record<string, Record<string, string>> = {};
        try { if (fs.existsSync(PICTURE_CACHE)) cache = JSON.parse(fs.readFileSync(PICTURE_CACHE, 'utf8')); } catch { /* ignore */ }
        ...
        try {
            ...
            const key = ((keyResp.data ?? {}) as Record<string, string>).key;
            ...
            zip.getEntries().forEach((e: { entryName: string; getData: () => Buffer }) => {
    ...
    private async writePictures(vin: string, pics: Record<string, string>): Promise<void> {
        for (const [name, data] of Object.entries(pics)) {
            await this.ensureAndSet(`${vin}.pictures.${name}`, data, 'string', '', 'Bild: ' + name);
        }
    }

    // ── Composite HTML Dashboard ─────────────────────────────

    private async buildCompositeHtml(vin: string, s: Record<string, unknown>): Promise<void> {
        ...
        const windowOpen = ((s.leftFrontWindowPercent as number) > 0) || ((s.rightFrontWindowPercent as number) > 0) ||
                           ((s.leftRearWindowPercent  as number) > 0) || ((s.rightRearWindowPercent  as number) > 0);
        const charging = s.chargeState != null && [1, 2, 3].includes(s.chargeState as number);
        const plugged  = (s.chargeState as number) > 0;
        ...
        const tile = (label: string, val: string, color: string) =>
            `<div style="background:${C.bg2};border:1px solid ${C.border};border-radius:10px;padding:10px;text-align:center">` +
            `<div style="font-size:8px;color:${C.textDim};letter-spacing:0.1em;margin-bottom:3px">${label}</div>` +
            `<div style="font-size:12px;font-weight:800;color:${color}">${val}</div></div>`;

        const speed = (s.speed as number) || 0;
        const tiles =
            `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:8px">` +
            tile('AUSSEN', `${s.outdoorTemp || 0}°C`, C.text) +
            tile('REICHWEITE', `${s.expectedMileage || 0} km`, C.accent) +
            tile('STATUS', speed === 0 ? '🅿 Geparkt' : `▶ ${speed} km/h`, speed === 0 ? C.green : C.yellow) +
        ...
        // Buttons via servConn.setState
        const ns = `leapmotor.${this.instance}.${vin}`;
        const sdp = (dp: string, val: string) => `servConn.setState('${ns}.${dp}',${val})`;
        const bs = (color: string, active: boolean) =>
            `background:${active ? color + '22' : C.bg2};border:1px solid ${active ? color + '55' : C.border};` +
            `border-radius:10px;padding:10px 4px;color:${active ? color : C.textDim};font-size:10px;font-weight:700;` +
            `cursor:pointer;font-family:monospace;display:flex;flex-direction:column;align-items:center;gap:3px;width:100%`;

        const acTemp = (s.acSetting as number) || 22;
        const acMode = acOn ? (acTemp >= 23 ? 'kuehl' : acTemp < 20 ? 'heiz' : 'luft') : '';
        ...
        const tempRow =
            `<div style="display:flex;align-items:center;justify-content:space-between;background:${C.bg2};border-radius:10px;padding:8px 12px;margin-bottom:8px;border:1px solid ${C.border}">` +
            `<span style="font-size:9px;color:${C.textDim};letter-spacing:0.12em;text-transform:uppercase">Zieltemperatur</span>` +
            `<div style="display:flex;align-items:center;gap:12px">` +
            `<button onclick="${sdp('cmd.ac_temp', String(Math.max(16, acTemp-1)))}" style="background:${C.border};border:none;border-radius:6px;color:${C.text};font-size:18px;width:30px;height:30px;cursor:pointer;font-weight:700">−</button>` +
            `<span style="font-size:18px;font-weight:800;color:${C.accent};min-width:40px;text-align:center">${acTemp}°C</span>` +
            `<button onclick="${sdp('cmd.ac_temp', String(Math.min(30, acTemp+1)))}" style="background:${C.border};border:none;border-radius:6px;color:${C.text};font-size:18px;width:30px;height:30px;cursor:pointer;font-weight:700">+</button>` +
            `</div></div>`;
        ...
        const html =
            `<div style="font-family:monospace;background:${C.bg};border-radius:18px;overflow:hidden;border:1px solid #1a2a40;box-shadow:0 24px 64px rgba(0,0,0,0.6)">` +
            `<div style="padding:12px 14px 0;display:flex;justify-content:space-between;align-items:flex-end">` +
            `<div><div style="font-size:9px;color:${C.textDim};letter-spacing:0.25em;text-transform:uppercase;margin-bottom:2px">LEAPMOTOR T03</div>` +
            `<div style="font-size:20px;font-weight:800;color:${C.text}">Kn&ouml;psel</div></div>` +
            `<div style="text-align:right"><div style="font-size:9px;color:${C.textDim};letter-spacing:0.15em;margin-bottom:2px">AKKUSTAND</div>` +
            `<div style="font-size:16px;font-weight:700;color:${socColor}">${soc}%</div></div></div>` +
            ...
            `<div style="position:absolute;top:8px;right:10px;font-size:10px;color:${C.textDim}">${s.outdoorTemp || 0}&deg;C</div></div>` +
            `<div style="padding:6px 14px 0"><div style="background:${C.bg2};border-radius:3px;height:4px;overflow:hidden">` +
            `<div style="height:100%;width:${soc}%;background:linear-gradient(90deg,${socColor}88,${socColor});border-radius:3px"></div></div></div>` +
            `<div style="padding:10px 14px 14px">${tiles}${climateRow}${tempRow}${lockRow}</div></div>`;
    ...
    private async createVehicleObjects(vehicle: Vehicle): Promise<void> {
        ...
        const states: Array<[string, string, ioBroker.CommonType, string, string]> = [
        ...
        for (const [id, name, type, role, unit] of states) {
        ...
        for (const cmd of cmds) {
            await this.setObjectNotExistsAsync(`${vehicle.vin}.cmd.${cmd}`, { type: 'state', common: { name: cmd, type: 'boolean', role: 'button', read: true, write: true, def: false }, native: {} });
        }
    ...
    private async writeStatusStates(vin: string, s: Record<string, unknown>): Promise<void> {
        const set = async (id: string, val: unknown) => {
            if (val !== null && val !== undefined) await this.setStateAsync(`${vin}.${id}`, { val: val as ioBroker.StateValue, ack: true });
        };
        const tire = (v: unknown) => v != null ? Math.round(v as number) / 100 : null;
        ...
        await set('status.battery_energy_kwh', s.dumpEnergy != null ? Math.round(s.dumpEnergy as number / 100) / 10 : null);
        ...
        await set('status.charging_active',    s.chargeState != null ? [1,2,3].includes(s.chargeState as number) : null);
        ...
        await set('status.charging_plugged',   s.chargeState != null ? (s.chargeState as number) > 0 : null);
    ...
    private async executeCommand(vehicle: Vehicle, cmd: string): Promise<void> {
        if (!this.client) return;
        ...
        const ac = (mode: string, op: string) => JSON.stringify({ circle:'out', mode, operate:op, position:'all', temperature:temp, windlevel:'3', wshld:'0' });
        const noPinCmds: Record<string, [string, string]> = {
            'ac_kuehl': ['170', ac('cold','manual')], 'ac_heiz':  ['170', ac('hot', 'manual')],
            'ac_luft':  ['170', ac('wind','manual')], 'ac_off':   ['170', ac('wind','off')],
            'defrost':  ['170', ac('wind','manual')], 'find':     ['120', '{}'],
            'battery_preheat':     ['190', '{"operate":"on"}'],
            'battery_preheat_off': ['190', '{"operate":"off"}'],
            'windows_open':        ['230', '{"value":"100"}'],
            'windows_close':       ['230', '{"value":"0"}'],
        };
        const pinCmds: Record<string, [string, string]> = {
            'lock': ['110', '{}'], 'unlock': ['110', '{"operate":"unlock"}'],
            'trunk_open': ['130', '{"operate":"open"}'], 'trunk_close': ['130', '{"operate":"close"}'],
        };
        try {
...
if (require.main !== module) {
    module.exports = (options: Partial<utils.AdapterOptions>) => new LeapmotorAdapter(options);
} else {
